...
$(document).ready(function() {
	//返回
	/*$(".exam-back-btn").click(function(e) {
        document.location.href = "/exam/m_user_index/";
    });
	//显示答题卡
	$(".exam-showNum-btn").click(function(e) {
		e.stopPropagation();
		e.preventDefault();
		$("#numHeader , #numPanel").show();
		$("#examHeader , #examPanel").hide();
    });
	//隐藏答题卡
	$(".num-back-btn").click(function(e) {
		e.stopPropagation();
		e.preventDefault();
		$("#numHeader , #numPanel").hide();
		$("#examHeader , #examPanel").show();
    });
	//试题内容第一个P标签加样式
	$(".answers p:first , .questions p:first").css("display","inline");
	//判断是否答过，标记为已答
	...
	$("a.iconBox").click(function(e) {
		...
		$(".iconBoxNum").each(function(index, element) {
			if($(this).attr("questionsId")==nowId){
				if(index==0){
					return loadQuestionsFn("","");
				}else{
					nowId = $($(".iconBoxNum")[index-1]).attr("questionsId");
					return loadQuestionsFn(nowId,"next");
				}
			}
		});
		$(".num-back-btn").click();
	...
	$("#nextQuestions").click(function(e) {
		e.stopPropagation();
		e.preventDefault();
		loadQuestionsFn($(this).attr("nowId"),"next");
    });
	//交卷
	...
	$(".examEndJumpBtn").click(function(e) {
		e.stopPropagation();
		e.preventDefault();
		window.location.href = "/exam/m_listexam.html"; 
    });
});
//倒计时
...
function loadQuestionsFn(nowId,setp){
	...
	var setp = setp;
	if(nowId==""){
		var nextId = $($(".iconBoxNum")[0]).attr("questionsId");
		loadFn(nextId,0);
		return;
	}
	$(".iconBoxNum").each(function(index, element) {
        if($(this).attr("questionsId")==nowId){
			var nextId = "";
			if(setp=="pre"){
				nextId = $($(".iconBoxNum")[index-1]).attr("questionsId");
			}else{
				nextId = $($(".iconBoxNum")[index+1]).attr("questionsId");
			}
			//判断第一题或最后一题
			...
			if(index==1&&setp=="pre"){
				num = 0;
			}else if((index+2)==$(".iconBoxNum").length && setp!="pre"){
				num = 1;
			}
			return loadFn(nextId,num);
		}
    });
	function loadFn(questionsId,index){
		setQuestionsIndex(questionsId,index);
		loadingShowOrHide("hide");
		var dataForm = "exam_results_id="+exam_results_id+"&exam_info_id="+exam_info_id+"&test_id="+questionsId;
		$.ajax({
			type: "POST",
			dataType: "json",
			url: "/exam/m_exam_start_test",
			data: dataForm,
			success: function(msg){
				createQuestionsDom(msg);
				
			}
		});
	}
}*/
//loadQuestionsFn("",""); //第一道题初始化
